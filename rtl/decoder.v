    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            case (inst_opcode)
                ...
                //============================================================
                // Blackwell 5th-gen Tensor Core (tcgen05) - SM100+
                // Per-thread tensor operations with TMEM accumulator
                //============================================================
                `OP_TCGEN05: begin
                    tcgen05_op <= 1'b1;
                    case (inst_func)
                        `TCGEN05_MMA: begin
                            // tcgen05.mma d-tmem, a-desc, b-desc, idesc
                            // Per-thread async MMA with TMEM accumulator
                            // rd = TMEM address, ra = A descriptor, rb = B descriptor
                            tcgen05_mma <= 1'b1;
                            mem_shared <= 1'b1;  // Operands come from shared memory
                            // No reg_write - result goes to TMEM
                            `ifdef SIMULATION
                            $display("[DECODER] TCGEN05_MMA: tmem_addr=0x%04x dtype=%0d", inst_imm16, inst_rc[3:0]);
                            `endif
                        end
                        `TCGEN05_LD: begin
                            ...
                            reg_write <= 1'b1;
                            `ifdef SIMULATION
                            $display("[DECODER] TCGEN05_LD: rd=R%0d tmem_addr=0x%04x", inst_rd, inst_imm16);
                            `endif
                        end
                        `TCGEN05_ST: begin
                            // tcgen05.st [tmem_addr], rs
                            // Store from register to TMEM
                            tcgen05_st <= 1'b1;
                            `ifdef SIMULATION
                            $display("[DECODER] TCGEN05_ST: tmem_addr=0x%04x ra=R%0d", inst_imm16, inst_ra);
                            `endif
                        end
                        `TCGEN05_CP: begin
                            // tcgen05.cp dst_tmem, src_smem, size
                            // Async tensor data transfer (TMA-like)
                            tcgen05_cp <= 1'b1;
                            mem_shared <= 1'b1;  // Source is shared memory
                            `ifdef SIMULATION
                            $display("[DECODER] TCGEN05_CP: tmem=0x%04x smem=R%0d", inst_imm16, inst_ra);
                            `endif
                        end
                        `TCGEN05_ALLOC: begin
                            // tcgen05.alloc rd, num_cols
                            // Allocate TMEM columns, return base address in rd
                            tcgen05_alloc <= 1'b1;
                            reg_write <= 1'b1;  // Returns TMEM base address
                            `ifdef SIMULATION
                            $display("[DECODER] TCGEN05_ALLOC: rd=R%0d cols=%0d", inst_rd, inst_imm16[8:0]);
                            `endif
                        end
                        `TCGEN05_DEALLOC: begin
                            // tcgen05.dealloc tmem_base
                            // Deallocate TMEM (required before kernel exit)
                            tcgen05_dealloc <= 1'b1;
                            `ifdef SIMULATION
                            $display("[DECODER] TCGEN05_DEALLOC: base=R%0d", inst_ra);
                            `endif
                        end
                        `TCGEN05_COMMIT: begin
                            // tcgen05.commit mbar_addr
                            // Signal MMA completion via mbarrier
                            tcgen05_commit <= 1'b1;
                            mbarrier_op <= 1'b1;  // Uses mbarrier infrastructure
                            `ifdef SIMULATION
                            $display("[DECODER] TCGEN05_COMMIT: mbar=R%0d", inst_ra);
                            `endif
                        end
                        `TCGEN05_WAIT: begin
                            // tcgen05.wait
                            // Wait for all pending TMEM operations
                            tcgen05_wait <= 1'b1;
                            sync_op <= 1'b1;  // Stalls warp until complete
                            `ifdef SIMULATION
                            $display("[DECODER] TCGEN05_WAIT");
                            `endif
                        end
                        default: begin
                            // Unknown tcgen05 operation - treat as NOP
                            `ifdef SIMULATION
