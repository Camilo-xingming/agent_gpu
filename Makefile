...
# 运行所有测试
#----------------------------------------------------------------------------
test: test_alu test_mul test_fma_int32 test_decoder test_regfile test_smem test_memory_coalescing_unit test_memory_interface test_sm_fetch_pipeline test_sm_writeback_arbiter test_sm_gmem_arbiter test_texture_smem_gmem_integration test_sm_special_reg test_memory_qos test_l2_interconnect test_reconvergence_stack test_lz4_decompressor test_chi_controller test_warp test_atomic test_mbarrier_unit test_tma_unit test_multimem_unit test_cluster_barrier_unit test_warp_shuffle test_warp_collective_unit test_dual_issue_scheduler test_warp_ops test_video_unit test_fpu test_fp16_unit test_fpu64 test_sfu test_cvt_unit test_dpx_unit test_control_flow_unit test_branch_predictor test_icache test_cache_policy_unit test_forwarding_unit test_l1_data_cache_optimized test_tensor_core_e2e test_tensor_memory test_texture_unit test_command_queue test_command_processor test_wb_fifo test_sm_wbq_bank test_memory_controller_hbm test_async_copy_engine test_advanced_scheduler test_wgmma_throughput test_wgmma_tile_engine test_tensor_core_fp4 test_memory_controller test_tensor_fp4_fp8
	@echo "========================================"
...
# Added in #463: memory subsystem + sync unit TB targets
TB_MBARRIER_UNIT = $(TB_DIR)/tb_mbarrier_unit.v
TB_TMA_UNIT = $(TB_DIR)/tb_tma_unit.v
TB_MULTIMEM_UNIT = $(TB_DIR)/tb_multimem_unit.v
TB_CLUSTER_BARRIER_UNIT = $(TB_DIR)/tb_cluster_barrier_unit.v
TB_WARP_SHUFFLE = $(TB_DIR)/tb_warp_shuffle.v
...
$(BUILD_DIR)/tb_mbarrier_unit.vvp: $(RTL_DIR)/mbarrier_unit.v $(RTL_DIR)/gpu_defines.vh $(TB_MBARRIER_UNIT) | $(BUILD_DIR)
	$(IVERILOG) -g2012 $(INCLUDES) -o $@ $(TB_MBARRIER_UNIT) $(RTL_DIR)/mbarrier_unit.v
...
$(BUILD_DIR)/tb_tma_unit.vvp: $(RTL_DIR)/tma_unit.v $(RTL_DIR)/gpu_defines.vh $(TB_TMA_UNIT) | $(BUILD_DIR)
	$(IVERILOG) -g2012 $(INCLUDES) -o $@ $(TB_TMA_UNIT) $(RTL_DIR)/tma_unit.v
...
$(BUILD_DIR)/tb_multimem_unit.vvp: $(RTL_DIR)/multimem_unit.v $(RTL_DIR)/gpu_defines.vh $(TB_MULTIMEM_UNIT) | $(BUILD_DIR)
	$(IVERILOG) -g2012 $(INCLUDES) -o $@ $(TB_MULTIMEM_UNIT) $(RTL_DIR)/multimem_unit.v
...
$(BUILD_DIR)/tb_cluster_barrier_unit.vvp: $(RTL_DIR)/cluster_barrier_unit.v $(RTL_DIR)/gpu_defines.vh $(TB_CLUSTER_BARRIER_UNIT) | $(BUILD_DIR)
	$(IVERILOG) -g2012 $(INCLUDES) -o $@ $(TB_CLUSTER_BARRIER_UNIT) $(RTL_DIR)/cluster_barrier_unit.v
...
$(BUILD_DIR)/tb_warp_shuffle.vvp: $(RTL_DIR)/warp_shuffle.v $(RTL_DIR)/gpu_defines.vh $(TB_WARP_SHUFFLE) | $(BUILD_DIR)
	$(IVERILOG) -g2012 $(INCLUDES) -o $@ $(TB_WARP_SHUFFLE) $(RTL_DIR)/warp_shuffle.v
