...
module tb_alu;
    ...
    task check_result;
        ...
        begin
            #1;  // 等待组合逻辑稳定
            if (result === exp_result && zero === exp_zero && negative === exp_neg) begin
                $display("[PASS] %s: result=0x%08X, zero=%b, neg=%b",
                         test_name, result, zero, negative);
                passed = passed + 1;
            end else begin
                $display("[FAIL] %s: got 0x%08X (z=%b,n=%b), expected 0x%08X (z=%b,n=%b)",
                         test_name, result, zero, negative, exp_result, exp_zero, exp_neg);
                failed = failed + 1;
            end
        end
    endtask
    ...
    initial begin
        ...
        check_result(32'd300, 1'b0, 1'b0, "ADD 100+200");

        // 加零
        func = `FUNC_ADD; operand_a = 32'd50; operand_b = 32'd0;
        check_result(32'd50, 1'b0, 1'b0, "ADD 50+0");

        // 结果为零
        func = `FUNC_ADD; operand_a = 32'd0; operand_b = 32'd0;
        check_result(32'd0, 1'b1, 1'b0, "ADD 0+0");

        // 负数(补码)
        func = `FUNC_ADD; operand_a = 32'hFFFFFFFF; operand_b = 32'd1;
        check_result(32'd0, 1'b1, 1'b0, "ADD -1+1");

        // 大数相加
        func = `FUNC_ADD; operand_a = 32'h7FFFFFFF; operand_b = 32'd1;
        check_result(32'h80000000, 1'b0, 1'b1, "ADD MAX+1 (overflow)");
        ...
        check_result(32'd200, 1'b0, 1'b0, "SUB 300-100");

        func = `FUNC_SUB; operand_a = 32'd100; operand_b = 32'd100;
        check_result(32'd0, 1'b1, 1'b0, "SUB 100-100");

        func = `FUNC_SUB; operand_a = 32'd50; operand_b = 32'd100;
        check_result(32'hFFFFFFCE, 1'b0, 1'b1, "SUB 50-100 (negative)");
        ...
        check_result(32'h0F000F00, 1'b0, 1'b0, "AND patterns");

        func = `FUNC_AND; operand_a = 32'hFFFFFFFF; operand_b = 32'h00000000;
        check_result(32'h00000000, 1'b1, 1'b0, "AND all-ones & zeros");
        ...
        check_result(32'hFFFFFFFF, 1'b0, 1'b1, "OR patterns");

        func = `FUNC_OR; operand_a = 32'h00000000; operand_b = 32'h00000000;
        check_result(32'h00000000, 1'b1, 1'b0, "OR zeros");
        ...
        check_result(32'hFFFFFFFF, 1'b0, 1'b1, "XOR alt patterns");

        func = `FUNC_XOR; operand_a = 32'h12345678; operand_b = 32'h12345678;
        check_result(32'h00000000, 1'b1, 1'b0, "XOR same value");
        ...
        check_result(32'h00000000, 1'b1, 1'b0, "NOT all-ones");

        func = `FUNC_NOT; operand_a = 32'h00000000; operand_b = 32'd0;
        check_result(32'hFFFFFFFF, 1'b0, 1'b1, "NOT zeros");
        ...
        check_result(32'd16, 1'b0, 1'b0, "SHL 1<<4");

        func = `FUNC_SHL; operand_a = 32'h80000000; operand_b = 32'd1;
        check_result(32'h00000000, 1'b1, 1'b0, "SHL overflow");

        func = `FUNC_SHL; operand_a = 32'h00000001; operand_b = 32'd31;
        check_result(32'h80000000, 1'b0, 1'b1, "SHL to MSB");
        ...
        check_result(32'd1, 1'b0, 1'b0, "SHR_U 16>>4");

        func = `FUNC_SHR_U; operand_a = 32'h80000000; operand_b = 32'd4;
        check_result(32'h08000000, 1'b0, 1'b0, "SHR_U sign bit");
        ...
        check_result(32'hF8000000, 1'b0, 1'b1, "SHR_S sign extend");

        func = `FUNC_SHR_S; operand_a = 32'h7FFFFFFF; operand_b = 32'd4;
        check_result(32'h07FFFFFF, 1'b0, 1'b0, "SHR_S positive");
        ...
        check_result(32'hFFFFFFFE, 1'b0, 1'b1, "ADD max+max");

        // 移位边界
        func = `FUNC_SHL; operand_a = 32'hFFFFFFFF; operand_b = 32'd0;
        check_result(32'hFFFFFFFF, 1'b0, 1'b1, "SHL by 0");

        func = `FUNC_SHL; operand_a = 32'hFFFFFFFF; operand_b = 32'd32;
        check_result(32'hFFFFFFFF, 1'b0, 1'b1, "SHL by 32 (uses low 5 bits)");
